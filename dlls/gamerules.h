...
public:
        ...
        // End-game sequence
        void End( CBasePlayer *pPlayer );
        void OnEnd( CBasePlayer *pPlayer );
        bool ended;
        float m_flMapStartTime;

        // Map commentary hook system (ported from Half-Payne GoldSRC)
        bool m_bMapStartHookFired;
        virtual void HookModelIndex( edict_t *activator );
        virtual void HookModelIndex( int modelIndex, const char *className, const char *targetName );
        virtual void OnHookedModelIndex( CBasePlayer *pPlayer, int modelIndex,
                const char *className, const char *targetName, bool firstTime );
};

// Free function callable from entity code without knowing the gamerules subclass
extern void HalfPayneHookModelIndex( edict_t *edict );
