...
void CBaseMonster::Killed( entvars_t *pevAttacker, int iGib )
{
        ...
        // Notify the player who killed this monster (for crystal/G-Man voice system)
        if( pevAttacker )
        {
                CBaseEntity *pKiller = CBaseEntity::Instance( pevAttacker );
                if( pKiller && pKiller->IsPlayer() )
                {
                        static_cast<CBasePlayer *>( pKiller )->OnKilledEntity( this );
                }
        }
...
Vector CBaseEntity::FireBulletsPlayer( ULONG cShots, Vector vecSrc, Vector vecDirShooting, Vector vecSpread, float flDistance, int iBulletType, int iTracerFreq, int iDamage, entvars_t *pevAttacker, int shared_rand )
{
        ...
        for( ULONG iShot = 1; iShot <= cShots; iShot++ )
        {
                ...
                if( tr.flFraction != 1.0f )
                {
                        ...
                        {
                                if( pevAttacker && FStrEq( STRING( pevAttacker->classname ), "player" ) )
                                {
                                        CBasePlayer *pShooter = static_cast<CBasePlayer *>( CBaseEntity::Instance( pevAttacker ) );
                                        if( pShooter ) pShooter->OnBulletHit( pEntity );
                                }
                        }
