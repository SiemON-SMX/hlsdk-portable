...
#define WEAPON_CROWBAR                  14
#define WEAPON_GLOCK                    15
#define WEAPON_PYTHON                   16
#define WEAPON_MP5                      17
#define WEAPON_CHAINGUN                 18
#define WEAPON_CROSSBOW                 19
#define WEAPON_RPG                      20
#define WEAPON_GAUSS                    21
#define WEAPON_EGON                     22
#define WEAPON_HORNETGUN                23
#define WEAPON_HANDGRENADE              24
#define WEAPON_TRIPMINE                 25
#define WEAPON_SATCHEL                  26
#define WEAPON_SNARK                    27
...
// WantedHL forward declarations
class CDynamite : public CGrenade
{
public:
        void Spawn( void );
        static CGrenade *ShootTimed( entvars_t *pevOwner, Vector vecStart, Vector vecVelocity, float flTime );
};
...
public:
        ...
        int GetItemInfo( ItemInfo *p );
        void PrimaryAttack( void );
        void SecondaryAttack( void );   // throw the knife
        void EXPORT CreateThink( void ); // deferred throw — spawns projectile, removes weapon
        BOOL Deploy( void );
        void Holster( int skiplocal = 0 );
        void WeaponIdle( void );
        int Swing( int fFirst );
        void SwingAgain( void ) { Swing( 0 ); }
private:
...
enum QFSTATE_e { QFSTATE_NONE = 0, QFSTATE_READY, QFSTATE_SHOOT, QFSTATE_RELAX };
...
private:
        ...
        BOOL m_fInZoom;   // TRUE while FOV is narrowed to BUFFALO_ZOOM_FOV
};
...
class CGattlingGun : public CBasePlayerWeapon
{
public:
        void Spawn( void );
        void Precache( void );
...
private:
        int   m_iShell;
        unsigned short m_usGattlingGun;
        // Barrel spin state — do NOT use pev->sequence for this; pev->sequence
        // is the weapon entity's own field and is never written by SendWeaponAnim.
        // 0=stopped  1=spinup  2=fire  3=spindown  4=jammed
        int   m_iGatlingState;
        float m_flSpinTime;   // time when the current state started
        BOOL  m_bJammed;      // TRUE when the barrel is jammed; unjam via secondary attack
};

class CBeartrap : public CBasePlayerWeapon
{
public:
        void Spawn( void );
        void Precache( void );
