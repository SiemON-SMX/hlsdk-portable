...
void CShotgun2::ShotgunFire(float flSpread, float flCycleTime, float flIdleTime, BOOL fUseAutoAim, BOOL fSecondary)
{
        ...
        SendWeaponAnim(fSecondary ? DBARREL_SHOOT2 : DBARREL_SHOOT, UseDecrement());

        // Fire sound — direct so it always plays regardless of event system
        if (fSecondary)
                EMIT_SOUND_DYN(ENT(m_pPlayer->pev), CHAN_WEAPON, "weapons/dbarrel1.wav", 1, ATTN_NORM, 0, 93 + RANDOM_LONG(0, 0xf));
        else
                EMIT_SOUND_DYN(ENT(m_pPlayer->pev), CHAN_WEAPON, "weapons/sbarrel1.wav", 1, ATTN_NORM, 0, 93 + RANDOM_LONG(0, 0xf));
...
#endif
        {
                ...
                vecDir = m_pPlayer->FireBulletsPlayer(!fSecondary ? 4 : 8, vecSrc, vecAiming, VECTOR_CONE_DM_DOUBLESHOTGUN, 2048, BULLET_PLAYER_BUCKSHOT, 0, (int)gSkillData.plrDmgWantedShotgun, m_pPlayer->pev, m_pPlayer->random_seed);
        }
        else
        {
                // untouched default single player
                vecDir = m_pPlayer->FireBulletsPlayer(!fSecondary ? 8 : 12, vecSrc, vecAiming, VECTOR_CONE_10DEGREES, 2048, BULLET_PLAYER_BUCKSHOT, 0, (int)gSkillData.plrDmgWantedShotgun, m_pPlayer->pev, m_pPlayer->random_seed);
        }
