...
void CPistol::Precache(void)
{
        ...
        m_iShellPistol = PRECACHE_MODEL("models/shell.mdl");// brass shell
...
void CPistol::GlockFire(float flSpread, float flCycleTime, BOOL fUseAutoAim, BOOL fSecondary)
{
        ...
        SendWeaponAnim(PISTOL_SHOOT, UseDecrement());

        // Fire sound — direct so it always plays regardless of event system
        if (RANDOM_LONG(0, 1))
                EMIT_SOUND_DYN(ENT(m_pPlayer->pev), CHAN_WEAPON, "weapons/pistol_shot1.wav", 1, ATTN_NORM, 0, 94 + RANDOM_LONG(0, 0xf));
        else
                EMIT_SOUND_DYN(ENT(m_pPlayer->pev), CHAN_WEAPON, "weapons/pistol_shot2.wav", 1, ATTN_NORM, 0, 94 + RANDOM_LONG(0, 0xf));
        ...
        Vector vecDir;
        vecDir = m_pPlayer->FireBulletsPlayer(1, vecSrc, vecAiming, Vector(flSpread, flSpread, flSpread), 8192, BULLET_PLAYER_9MM, 0, (int)gSkillData.plrDmgPistolBullet, m_pPlayer->pev, m_pPlayer->random_seed);
