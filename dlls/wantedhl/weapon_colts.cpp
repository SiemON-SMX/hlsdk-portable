...
void CColts::ColtsFire(float flSpread, float flCycleTime, BOOL fDualFire, BOOL fLeftAttack)
{
        ...
        if (fDualFire)
                SendWeaponAnim(COLTS_DUALFIRE, UseDecrement());
        else if (fLeftAttack)
                SendWeaponAnim(COLTS_LEFTFIRE, UseDecrement());
        else
                SendWeaponAnim(COLTS_RIGHTFIRE, UseDecrement());

        // Fire sound — direct so it always plays regardless of event system
        if (RANDOM_LONG(0, 1))
                EMIT_SOUND_DYN(ENT(m_pPlayer->pev), CHAN_WEAPON, "weapons/coltsfire1.wav", 1, ATTN_NORM, 0, 94 + RANDOM_LONG(0, 0xf));
        else
                EMIT_SOUND_DYN(ENT(m_pPlayer->pev), CHAN_WEAPON, "weapons/coltsfire2.wav", 1, ATTN_NORM, 0, 94 + RANDOM_LONG(0, 0xf));

        Vector vecSrc = m_pPlayer->GetGunPosition();
        Vector vecAiming = m_pPlayer->GetAutoaimVector(AUTOAIM_2DEGREES);

        Vector vecDir = m_pPlayer->FireBulletsPlayer(1, vecSrc, vecAiming, Vector(flSpread, flSpread, flSpread), 8192, BULLET_PLAYER_9MM, 0, (int)gSkillData.plrDmgColtsBullet, m_pPlayer->pev, m_pPlayer->random_seed);
