...
// Zoom FOV for secondary attack (sniper scope)
#define WINCHESTER_ZOOM_FOV             20
...
int CWinchester::GetItemInfo(ItemInfo *p)
{
        ...
        p->iMaxAmmo1 = WINCHESTER_MAX_CARRY;
        p->pszAmmo2 = NULL;
        p->iMaxAmmo2 = -1;
...
void CWinchester::Holster(int skiplocal /* = 0 */)
{
        // Always reset zoom when holstering
        if (m_fInZoom)
        {
        ...
        SendWeaponAnim(WINCHESTER_RETHOLSTER, UseDecrement());
}
...
void CWinchester::PrimaryAttack()
{
        ...
        // Fire animation on viewmodel — done directly so it always plays regardless of event system
        SendWeaponAnim(WINCHESTER_FIRE, UseDecrement());

        // Fire sound — random variant for variety
        switch (RANDOM_LONG(0, 2))
        {
        case 0:
                EMIT_SOUND_DYN(ENT(m_pPlayer->pev), CHAN_WEAPON, "weapons/winchester_fire1.wav", 1, ATTN_NORM, 0, 93 + RANDOM_LONG(0, 0xf));
                break;
        case 1:
                EMIT_SOUND_DYN(ENT(m_pPlayer->pev), CHAN_WEAPON, "weapons/winchester_fire2.wav", 1, ATTN_NORM, 0, 93 + RANDOM_LONG(0, 0xf));
                break;
        default:
                EMIT_SOUND_DYN(ENT(m_pPlayer->pev), CHAN_WEAPON, "weapons/winchester_fire3.wav", 1, ATTN_NORM, 0, 93 + RANDOM_LONG(0, 0xf));
                break;
        }
...
#endif
        {
                vecDir = m_pPlayer->FireBulletsPlayer(1, vecSrc, vecAiming, VECTOR_CONE_1DEGREES, 2048, BULLET_PLAYER_357, 0, (int)gSkillData.plrDmgWinchesterBullet, m_pPlayer->pev, m_pPlayer->random_seed);
        }
        else
        {
                vecDir = m_pPlayer->FireBulletsPlayer(1, vecSrc, vecAiming, VECTOR_CONE_1DEGREES, 2048, BULLET_PLAYER_357, 0, (int)gSkillData.plrDmgWinchesterBullet, m_pPlayer->pev, m_pPlayer->random_seed);
        ...
        // Keep the event for any client-side effects (shell eject, etc.)
        ...
        // fire anim is 20 frames at 20fps = 1.0 sec; delay matches that
...
void CWinchester::SecondaryAttack(void)
{
        ...
        {
                // Zoom in — sniper FOV
                m_pPlayer->pev->fov = m_pPlayer->m_iFOV = WINCHESTER_ZOOM_FOV;
                m_fInZoom = TRUE;
        }
...
void CWinchester::Reload(void)
{
        ...
        // Reset zoom before reloading so the player can see what they're doing
        if (m_fInZoom)
        {
                m_pPlayer->pev->fov = m_pPlayer->m_iFOV = 0;
                m_fInZoom = FALSE;
...
void CWinchester::WeaponIdle(void)
{
        ...
        {
                // lever-action pump sound + animation after firing.
                // Previously only the sound played; the CLOSE_BREAK animation was never
                // sent here, so the lever was never visually worked after a shot.
        ...
        if (m_flTimeWeaponIdle < UTIL_WeaponTimeBase())
        {
                ...
                else if (m_fInSpecialReload != 0)
                {
                        ...
                        {
                                // reload debounce has timed out — close the action
                                SendWeaponAnim(WINCHESTER_CLOSE_BREAK, UseDecrement());
                ...
                {
                        SendWeaponAnim(WINCHESTER_IDLE, UseDecrement());
