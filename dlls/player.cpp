...
// using_sys_timescale declared in game.h (included above); no separate extern needed here.
...
TYPEDESCRIPTION CBasePlayer::m_playerSaveData[] =
{
        ...
        DEFINE_FIELD( CBasePlayer, slowMotionNextHeartbeatSound,FIELD_TIME  ),
        DEFINE_FIELD( CBasePlayer, deathCameraYaw,              FIELD_FLOAT ),
        DEFINE_FIELD( CBasePlayer, slowmotionInfiniteCheat,     FIELD_BOOLEAN ),
        ...
        DEFINE_FIELD( CBasePlayer, latestMaxCommentaryTime,          FIELD_TIME ),
        DEFINE_FIELD( CBasePlayer, allowedToReactOnPainkillerPickup, FIELD_TIME ),
        DEFINE_FIELD( CBasePlayer, allowedToReactOnPainkillerTake,   FIELD_TIME ),
        DEFINE_FIELD( CBasePlayer, allowedToReactOnPainkillerNeed,   FIELD_TIME ),
        DEFINE_FIELD( CBasePlayer, allowedToReactOnMinorInjury,      FIELD_TIME ),
        DEFINE_FIELD( CBasePlayer, allowedToReactOnSeriousInjury,    FIELD_TIME ),

        DEFINE_FIELD( CBasePlayer, dumbShots,                               FIELD_INTEGER ),
        DEFINE_FIELD( CBasePlayer, readyToComplainAboutDumbShots,           FIELD_CHARACTER ),
        DEFINE_FIELD( CBasePlayer, allowedToComplainAboutDumbShots,         FIELD_TIME ),
        DEFINE_FIELD( CBasePlayer, allowedToComplainAboutKillingInnocent,   FIELD_TIME ),
        DEFINE_FIELD( CBasePlayer, allowedToComplainAboutNoAmmo,            FIELD_TIME ),
        DEFINE_ARRAY( CBasePlayer, soundQueueIsImportant,                   FIELD_CHARACTER, MAX_SOUND_QUEUE ),
...
void LinkUserMessages( void )
{
        ...
        gmsgTwinClip = REG_USER_MSG( "TwinClip", 1 );
...
int CBasePlayer::TakePainkiller( void )
{
        ...
        if( CVAR_GET_FLOAT( "max_commentary_painkiller_pickup" ) > 0.0f &&
            RANDOM_LONG( 0, 100 ) < 33 && gpGlobals->time > allowedToReactOnPainkillerPickup )
        {
                char fileName[256];
                snprintf( fileName, sizeof( fileName ), "max/painkiller/FIND_PILLS_%d.wav", (int)RANDOM_LONG( 1, 18 ) );
                TryToPlayMaxCommentary( fileName, FALSE );
...
void CBasePlayer::UsePainkiller( void )
{
        if( painkillerCount <= 0 )
                return;

        if( ( pev->health + painkillerEnergy ) < pev->max_health )
        {
                ...
                // White screen flash
                UTIL_ScreenFade( this, Vector( 255, 255, 255 ), 0.12f, 0.0f, 120, FFADE_IN );

                EMIT_SOUND( ENT( pev ), CHAN_ITEM, "items/pills_use.wav", 1, ATTN_NORM );

                if( CVAR_GET_FLOAT( "max_commentary_painkiller_use" ) > 0.0f &&
                    RANDOM_LONG( 0, 100 ) < 33 && gpGlobals->time > allowedToReactOnPainkillerTake && desperation == DESPERATION_NO )
                {
                        char fileName[256];
                        snprintf( fileName, sizeof( fileName ), "max/painkiller/TAKE_PILLS_%d.wav", (int)RANDOM_LONG( 1, 8 ) );
                        TryToPlayMaxCommentary( fileName, FALSE );
...
void CBasePlayer::ComplainAboutKillingInnocent( void )
{
        if( CVAR_GET_FLOAT( "max_commentary_kill_innocent" ) <= 0.0f )
                return;
        if( gpGlobals->time < allowedToComplainAboutKillingInnocent )
                return;
        if( RANDOM_LONG( 0, 100 ) >= 33 )
                return;

        char fileName[256];
        snprintf( fileName, sizeof( fileName ), "max/innocent_killed/INNOCENT_KILLED_%d.wav", (int)RANDOM_LONG( 1, 5 ) );
        TryToPlayMaxCommentary( fileName, FALSE );
...
{
        if( CVAR_GET_FLOAT( "max_commentary_no_ammo" ) <= 0.0f )
                return;
        if( gpGlobals->time < allowedToComplainAboutNoAmmo )
                return;
        ...
        if( weaponIsBulletBased )
                snprintf( fileName, sizeof( fileName ), "max/no_ammo/NO_AMMO_%d.wav", (int)RANDOM_LONG( 1, 20 ) );
        else
                snprintf( fileName, sizeof( fileName ), "max/no_ammo/NO_AMMO_%d.wav", (int)RANDOM_LONG( 1, 20 ) );
        TryToPlayMaxCommentary( fileName, FALSE );
...
{
        if( CVAR_GET_FLOAT( "max_commentary_wasted_shots" ) <= 0.0f )
                return;
        if( !hitEntity )
                return;

        if( hitEntity->pev->takedamage != DAMAGE_NO )
        {
                // Hits on damageable things reset the counter.
                dumbShots = 0;
        ...
        if( dumbShots >= 20 && !readyToComplainAboutDumbShots )
                readyToComplainAboutDumbShots = true;

        if( readyToComplainAboutDumbShots && gpGlobals->time > allowedToComplainAboutDumbShots )
        {
                char fileName[256];
                snprintf( fileName, sizeof( fileName ), "max/dumb_shoot/SHOOT_THING_%d.wav", (int)RANDOM_LONG( 1, 22 ) );
                TryToPlayMaxCommentary( fileName, FALSE );
                allowedToComplainAboutDumbShots  = gpGlobals->time + 30.0f;
                dumbShots                        = 0;
                readyToComplainAboutDumbShots    = false;
        }
}

void CBasePlayer::TryToPlayMaxCommentary( const char *soundFile, BOOL isImportant, BOOL shortDelay )
{
        bool recentCommentary = latestMaxCommentaryTime > 0 &&
                ( gpGlobals->time - latestMaxCommentaryTime < ( shortDelay ? 1.5f : 10.0f ) );

        if( !isImportant && recentCommentary )
                return;

        EMIT_SOUND( ENT( pev ), CHAN_AUTO, soundFile, 1.0f, ATTN_STATIC );
        latestMaxCommentaryTime = gpGlobals->time;
}

void CBasePlayer::AddToSoundQueue( string_t soundName, float delay, bool isMaxCommentary, bool isImportant )
{
        if( soundQueueCounter >= MAX_SOUND_QUEUE )
                soundQueueCounter = 0;

        soundQueueSoundNames[soundQueueCounter]  = soundName;
        soundQueueSoundDelays[soundQueueCounter] = gpGlobals->time + delay;
        soundQueueIsImportant[soundQueueCounter] = isImportant;
        soundQueueCounter++;
}

void CBasePlayer::CheckSoundQueue( void )
{
        for( int i = 0; i < MAX_SOUND_QUEUE; i++ )
        {
                if( soundQueueSoundNames[i] != 0 && gpGlobals->time >= soundQueueSoundDelays[i] )
                {
                        TryToPlayMaxCommentary( STRING( soundQueueSoundNames[i] ), soundQueueIsImportant[i] ? TRUE : FALSE );
                        soundQueueSoundNames[i]  = 0;
                        soundQueueSoundDelays[i] = 0;
                        soundQueueIsImportant[i] = false;
                }
        }
}

void CBasePlayer::ClearSoundQueue( void )
{
        for( int i = 0; i < MAX_SOUND_QUEUE; i++ )
        {
                soundQueueSoundNames[i]  = 0;
...
bool CBasePlayer::ModelIndexHasBeenHooked( const char *key )
{
        for ( int i = 0; i < hookedModelIndexCount; i++ )
        {
                if ( strcmp( STRING( hookedModelIndexKeys[i] ), key ) == 0 )
                        return true;
        }
        return false;
}

void CBasePlayer::RememberHookedModelIndex( string_t key )
{
        if ( hookedModelIndexCount < MAX_HOOKED_MODEL_INDEXES )
                hookedModelIndexKeys[hookedModelIndexCount++] = key;
}
...
void CBasePlayer::OnKilledEntity( CBaseEntity *victim )
{
        ...
        TakeSlowmotionCharge( 15 );

        const char *victimName = STRING( victim->pev->classname );

        // G-Man kill on c5a1 triggers the revenge/finale sequence
        if( strcmp( victimName, "monster_gman" ) == 0 )
        {
        ...
        // Innocent NPC (barney/scientist) kill commentary
        if( strcmp( victimName, "monster_barney" ) == 0 ||
            strcmp( victimName, "monster_scientist" ) == 0 )
        {
                ComplainAboutKillingInnocent();
        }

        // Nihilanth crystal destruction commentary (any map except c4a1)
        if( victim->pev->target != 0 &&
            strstr( STRING( victim->pev->target ), "crystal" ) &&
            strcmp( STRING( gpGlobals->mapname ), "c4a1" ) != 0 )
        {
                crystalsDestroyed++;
                switch( crystalsDestroyed )
                {
                case 1:
                        AddToSoundQueue( ALLOC_STRING( "comment/onedowntwotogo.wav" ), 0.6f );
                        break;
                case 2:
                        AddToSoundQueue( ALLOC_STRING( "comment/twodownonetogo.wav" ), 0.6f );
                        break;
                }
        }
}

void CBasePlayer::ThinkAboutFinalDesperation( void )
{
        ...
        if( untilNextDesperation && gpGlobals->time > untilNextDesperation )
        {
                ...
                case DESPERATION_PRE_IMMINENT:
                {
                        ...
                        while( ( pEntity = UTIL_FindEntityInSphere( pEntity, pev->origin, 4610.0f ) ) != NULL )
                        {
                                if( ( strcmp( STRING( pEntity->pev->classname ), "monster_alien_grunt" ) == 0 ||
                                      strcmp( STRING( pEntity->pev->classname ), "monster_alien_controller" ) == 0 ) &&
                ...
                case DESPERATION_IMMINENT:
                {
                        ...
                        GiveNamedItem( "weapon_ingram" );
                        GiveNamedItem( "weapon_ingram_twin" );
                ...
                case DESPERATION_RELIEF:
                {
                        ...
                        for( int i = 0; i < 4; i++ )
                        {
                                if( CBaseEntity *ent = CBaseEntity::Instance( edicts[i] ) )
                                        ent->Use( this, this, USE_SET, 1 );
                        }

                        CBaseEntity *teleport = CBaseEntity::Create( "trigger_teleport", VARS( edicts[0] )->origin, Vector( 0, 0, 0 ) );
                        if( teleport )
                        {
                                UTIL_SetSize( VARS( teleport->edict() ), Vector( 161, -1815, 1350 ), Vector( 221, -1755, 1410 ) );
                                teleport->Spawn();
                                teleport->pev->target = ALLOC_STRING( "backtrain" );
                        }
                        break;
                }
                ...
                case DESPERATION_REVENGE:
                {
                        ...
                        AddToSoundQueue( ALLOC_STRING( "comment/finalespeech.wav" ), 3.0f );
        ...
        // Kill player if they fall below the arena floor during the fight
        if( ( desperation == DESPERATION_FIGHTING || desperation == DESPERATION_RELIEF ) &&
            pev->origin.z < 1220 )
        {
                TakeDamage( pev, pev, 9999.9f, DMG_GENERIC );
        }

        // Check if all aliens are dead — transition FIGHTING → RELIEF
        ...
        if( !theyAreAlive && desperation == DESPERATION_FIGHTING )
        {
                ...
                RemoveAllItems( FALSE );
                AddToSoundQueue( ALLOC_STRING( "comment/finalewon.wav" ), 1.6f );
        }
}
...
int CBasePlayer::TakeDamage( entvars_t *pevInflictor, entvars_t *pevAttacker, float flDamage, int bitsDamageType )
{
        ...
        // Delay health regeneration for 3 seconds after taking damage (ported from HalfPayne)
        if( fTookDamage )
                m_flNextAutoHealTime = gpGlobals->time + 3.0f;

        if( flDamage > 0 && ( bitsDamageType & ( DMG_BULLET | DMG_SLASH | DMG_CLUB | DMG_SHOCK | DMG_SONIC | DMG_ENERGYBEAM ) ) )
        {
                int slowmoChargeGain = (int)( flDamage / 2.0f );
                if( slowmoChargeGain < 1 )
                        slowmoChargeGain = 1;
                TakeSlowmotionCharge( slowmoChargeGain );
        }
        ...
        // Max Payne pain voice reaction
        if( fTookDamage && pev->health > 0 )
        {
                if( bitsDamageType & ( DMG_BULLET | DMG_BLAST | DMG_FALL | DMG_SHOCK | DMG_CLUB | DMG_CRUSH | DMG_ENERGYBEAM | DMG_SLASH | DMG_SONIC ) )
                {
                        float dmgAmount = (float)m_lastDamageAmount;
                        bool isSerious = ( dmgAmount >= 30.0f );
                        bool timerOk = isSerious ? ( gpGlobals->time > allowedToReactOnSeriousInjury )
                                                 : ( gpGlobals->time > allowedToReactOnMinorInjury );

                        if( CVAR_GET_FLOAT( "max_commentary_pain" ) > 0.0f &&
                            ( ( bitsDamageType & DMG_FALL ) || ( timerOk && RANDOM_LONG( 0, 100 ) < 66 ) ) )
                        {
                                char fileName[256];
                                if( !isSerious )
                                {
                                        snprintf( fileName, sizeof( fileName ), "max/pain/MINOR_PAIN_%d.wav", (int)RANDOM_LONG( 1, 16 ) );
                                        EMIT_SOUND( ENT( pev ), CHAN_STATIC, fileName, 1, ATTN_NORM );
                                        allowedToReactOnMinorInjury = gpGlobals->time + 1.0f;
                                }
                                else
                                {
                                        if( bitsDamageType & DMG_FALL )
                                                snprintf( fileName, sizeof( fileName ), "max/pain/SERIOUS_PAIN_%d.wav", (int)RANDOM_LONG( 8, 13 ) );
                                        else
                                                snprintf( fileName, sizeof( fileName ), "max/pain/SERIOUS_PAIN_%d.wav", (int)RANDOM_LONG( 1, 13 ) );
                                        EMIT_SOUND( ENT( pev ), CHAN_STATIC, fileName, 1, ATTN_NORM );
                                        allowedToReactOnSeriousInjury = gpGlobals->time + 10.0f;

                                        if( CVAR_GET_FLOAT( "max_commentary_pain_self" ) > 0.0f &&
                                            ( ( bitsDamageType & DMG_FALL ) ||
                                              ( pAttacker && FStrEq( STRING( pAttacker->pev->classname ), "player" ) ) ) )
                                        {
                                                snprintf( fileName, sizeof( fileName ), "max/pain/SELF_PAIN_%d.wav", (int)RANDOM_LONG( 1, 20 ) );
                                                AddToSoundQueue( ALLOC_STRING( fileName ), 1.5f );
                                        }
                                }
                        }
                }
...
void CBasePlayer::Killed( entvars_t *pevAttacker, int iGib )
{
        ...
        // Restore full timescale immediately on death so the world doesn't stay slowed
        if ( slowMotionWasEnabled )
        {
                slowMotionWasEnabled      = false;
                nextSmoothTimeScaleChange = 0.0f;
                desiredTimeScale          = 1.0f;
                if ( using_sys_timescale )
                        SERVER_COMMAND( "sys_timescale 1\n" );
        }
        ...
        pev->skin = 1; // DEATH FACE - embedded in model (ported from GoldSRC)

        // Activate bullet-time slow-motion on death (matches GoldSRC behaviour).
        SetSlowMotion( true );
...
void CBasePlayer::PlayerDeathThink( void )
{
        ...
        pev->fixangle = 1;
        deathCameraYaw += 0.25f;
        if ( deathCameraYaw >= 360.0f )
                deathCameraYaw = 0.0f;
        ...
        // Return to first-person view and stop the death orbit before respawning.
        CVAR_SET_FLOAT( "cam_command", 0.0f );
        deathCameraYaw = 0.0f;
...
void CBasePlayer::PreThink( void )
{
        ...
        // Low-health regeneration (ported from HalfPayne): when health drops to 20 or below,
        // heal 1 HP per second back up to 20, but only after 3 seconds without taking damage.
        if( pev->health > 0.0f && pev->health <= 20.0f && m_flNextAutoHealTime <= gpGlobals->time )
        {
                float flMissing = 20.0f - pev->health;
                if( flMissing > 0.0f )
                {
                        TakeHealth( 1.0f, DMG_GENERIC );
                        m_flNextAutoHealTime = gpGlobals->time + 1.0f;
                }
        }

        UTIL_MakeVectors( pev->v_angle );             // is this still used?

        if( m_pQueuedItem && gpGlobals->time >= m_flItemSwitchTime )
        {
                CBasePlayerItem *pNew = m_pQueuedItem;
                m_pQueuedItem = NULL;

                if( m_pActiveItem )
                {
                        m_pActiveItem->pev->viewmodel = 0;
                        m_pActiveItem->pev->weaponmodel = 0;
                }
...
void CBasePlayer::UpdatePlayerSound( void )
{
        ...
        // Send world-space aim coordinates when cvar is enabled (dev tool)
        ...
        // Send OnAimNew/OnAimUpd for the entity under the crosshair when cvar is enabled
        if ( CVAR_GET_FLOAT( "print_aim_entity" ) >= 1.0f ) {
                ...
                if ( entity ) {
                        MESSAGE_BEGIN( MSG_ONE, gmsgOnAimUpd, NULL, pev );
                                ...
                                WRITE_SHORT( (int)entity->v.health );
                                WRITE_SHORT( (int)entity->v.max_health );

                                WRITE_BYTE( entity->v.flags );
                                WRITE_BYTE( entity->v.deadflag );
                        MESSAGE_END();
                }
        } else if ( aimLastEntity ) {
                MESSAGE_BEGIN( MSG_ONE, gmsgOnAimClear, NULL, pev );
                MESSAGE_END();
                aimLastEntity = 0;
        }
}
...
void CBasePlayer::PostThink()
{
            ...
            gpGlobals->time > allowedToReactOnPainkillerNeed )
        {
                char fileName[256];
                if( painkillerCount > 0 )
                        snprintf( fileName, sizeof( fileName ), "max/painkiller/HAS_PILLS_%d.wav", (int)RANDOM_LONG( 1, 6 ) );
                else
                        snprintf( fileName, sizeof( fileName ), "max/painkiller/NO_PILLS_%d.wav", (int)RANDOM_LONG( 1, 9 ) );
                TryToPlayMaxCommentary( fileName, FALSE );
...
void CBasePlayer::Spawn( void )
{
        ...
        // Painkiller system init
        painkillerCount          = 0;
        painkillerEnergy         = 0;
        nextPainkillerEffectTime = 0.0f;

        // Max Payne voice commentary init
        desperation                    = DESPERATION_NO;
        ClearSoundQueue();
        hookedModelIndexCount          = 0;
        memset( hookedModelIndexKeys, 0, sizeof( hookedModelIndexKeys ) );
        ...
        allowedToComplainAboutNoAmmo          = 0.0f;
        ...
        // --- Slow-motion system: reset all state on every spawn ---
        slowMotionCharge             = MAX_SLOWMOTION_CHARGE;
        slowMotionUpdateTime         = 1.0f;
        slowMotionRechargeTime       = 0.0f;
        slowMotionWasEnabled         = false;
        slowMotionNextHeartbeatSound = 0.0f;
        nextSmoothTimeScaleChange    = 0.0f;
        nextAttackSlowmotionOffset   = 0.0f;
        superHotMultiplier           = 0.0f;
        superHotJumping              = 0.0f;
        slowmotionInfiniteCheat      = false;
        desiredTimeScale             = 1.0f;
        m_flLastDesiredTimeScale     = 1.0f;
        deathCameraYaw               = 0.0f;
        // Force timescale back to normal and first-person on spawn.
        CVAR_SET_FLOAT( "cam_command", 0.0f );
        if ( using_sys_timescale )
                SERVER_COMMAND( "sys_timescale 1\n" );
        ...
        // Clear any lingering subtitles on spawn / level transition
        if( gmsgSubtClear )
        {
                MESSAGE_BEGIN( MSG_ONE, gmsgSubtClear, NULL, pev );
                MESSAGE_END();
        }
}
...
void CBasePlayer::SelectItem( const char *pstr )
{
        ...
        if( m_pActiveItem )
        {
                float fHolsterTime = 0.0f;
                CBasePlayerItem *pWeaponPtr = m_pActiveItem->GetWeaponPtr();
                if( pWeaponPtr )
                        fHolsterTime = ( (CBasePlayerWeapon *)pWeaponPtr )->GetHolsterDuration();

                if( fHolsterTime > 0.0f )
                {
                        m_pActiveItem->Holster();
                        m_pQueuedItem      = pItem;
                        m_flItemSwitchTime = gpGlobals->time + fHolsterTime;
                        return;
                }
...
void CBasePlayer::ImpulseCommands()
{
        ...
        // temporary flashlight for level designers
                ...
                break;
        case 22:
                // toggle slow-motion (bullet time)
                ToggleSlowMotion();
                break;
        case 23:
                // use a painkiller
...
void CBasePlayer::CheatImpulseCommands( int iImpulse )
{
        ...
        case 203:
                ...
                break;
        case 24:
                // Toggle infinite slow-motion cheat
                slowmotionInfiniteCheat = !slowmotionInfiniteCheat;
                if ( slowmotionInfiniteCheat )
...
#define clamp( val, min, max ) ( ((val) > (max)) ? (max) : ( ((val) < (min)) ? (min) : (val) ) )
                int iHealth = clamp( (int)pev->health, 0, 255 ); // make sure that no negative health values are sent
                if( pev->health > 0.0f && pev->health <= 1.0f )
                        iHealth = 1;

                // send "health" update message: LONG health + SHORT painkiller energy overlay
                MESSAGE_BEGIN( MSG_ONE, gmsgHealth, NULL, pev );
                        WRITE_LONG( iHealth );
        ...
        // Send painkiller count to client whenever it may have changed
        {
                MESSAGE_BEGIN( MSG_ONE, gmsgPainkillerCount, NULL, pev );
                        WRITE_BYTE( painkillerCount );
                MESSAGE_END();
        ...
        // --- Slow-motion: apply desired timescale only when it actually changed ---
        if ( using_sys_timescale && desiredTimeScale != m_flLastDesiredTimeScale )
        {
                char com[256];
                snprintf( com, sizeof(com), "sys_timescale %f\n", desiredTimeScale );
                SERVER_COMMAND( com );
                m_flLastDesiredTimeScale = desiredTimeScale;
        }

        // Send slow-motion charge level to HUD every frame
...
void CBasePlayer::TakeSlowmotionCharge( int amount )
{
        slowMotionCharge += amount;
        if ( slowMotionCharge > MAX_SLOWMOTION_CHARGE )
                slowMotionCharge = MAX_SLOWMOTION_CHARGE;
}

void CBasePlayer::HandleSlowmotionFlags()
{
        // placeholder – no FL_DIVING in this port
}
...
void CBasePlayer::ToggleSlowMotion()
{
        ...
        {
                if ( ActivateSlowMotion() && slowMotionWasEnabled )
                {
                        // Black screen flash on manual slow-motion activation
                        UTIL_ScreenFade( this, Vector( 0, 0, 0 ), 0.1f, 0.0f, 120, FFADE_IN );
...
bool CBasePlayer::ActivateSlowMotion()
{
        ...
        // Player must have the HEV suit
        if ( !( pev->weapons & ( 1 << WEAPON_SUIT ) ) )
                return false;

        // Need at least some charge
...
bool CBasePlayer::DeactivateSlowMotion( bool smooth )
{
        ...
        // Compensate m_flNextAttack for the time that was spent in slow-mo
...
void CBasePlayer::SetSlowMotion( BOOL slowMotionEnabled )
{
        ...
        {
                desiredTimeScale           = using_sys_timescale ? 0.25f : 0.25f;
        ...
        {
                // Only snap back immediately if we're not doing a smooth ramp-up
                if ( !nextSmoothTimeScaleChange )
                        desiredTimeScale = using_sys_timescale ? 1.0f : 1.0f;
                slowMotionWasEnabled = false;
        }
}

void CBasePlayer::UpdateSlowMotion()
{
        // --- Smooth timescale ramp-up after slow-mo ends ---
        ...
        // --- Drain the slow-motion charge while active ---
        if ( slowMotionUpdateTime && slowMotionUpdateTime <= gpGlobals->time && pev->deadflag == DEAD_NO )
        {
                if ( slowMotionWasEnabled )
                {
                        if ( slowmotionInfiniteCheat )
                        {
                                // cheat: keep full charge, just reset the timer
                                slowMotionCharge  = MAX_SLOWMOTION_CHARGE;
                                slowMotionUpdateTime = SLOWMOTION_DRAIN_TIME + gpGlobals->time;
                        }
                        else
        ...
        // --- Passively regain slow-motion charge over time while not in slow-mo ---
        if ( !slowMotionWasEnabled && pev->deadflag == DEAD_NO && slowMotionCharge < MAX_SLOWMOTION_CHARGE )
        {
                if ( slowMotionRechargeTime <= gpGlobals->time )
                {
                        slowMotionRechargeTime = gpGlobals->time + SLOWMOTION_RECHARGE_INTERVAL;

                        slowMotionCharge += SLOWMOTION_RECHARGE_AMOUNT;
                        if ( slowMotionCharge > MAX_SLOWMOTION_CHARGE )
                                slowMotionCharge = MAX_SLOWMOTION_CHARGE;
                }
        }
        else if ( slowMotionWasEnabled )
        {
                slowMotionRechargeTime = gpGlobals->time + SLOWMOTION_RECHARGE_INTERVAL;
        }

        // --- Heartbeat sound while in slow-motion ---
        if ( slowMotionWasEnabled && pev->deadflag == DEAD_NO )
        {
                if ( slowMotionNextHeartbeatSound <= gpGlobals->time )
                {
                        slowMotionNextHeartbeatSound = gpGlobals->time + 0.3f;
                        EMIT_SOUND( ENT( pev ), CHAN_AUTO, "slowmo/slowmo_heartbeat.wav", 1.0f, ATTN_NORM );
                }
        }
}
