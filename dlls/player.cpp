...
int CBasePlayer::Restore( CRestore &restore )
{
        ...
        // Push the active weapon's idle timer into the future so that
        // WeaponIdle() does not fire on the very first think frame after
        // restore.  Without this, WeaponIdle() calls SendWeaponAnim()
        // which sends SVC_WEAPONANIM before the client is reconnected
        // (changelevel), freezing the engine.
        if( m_pActiveItem )
        {
                CBasePlayerWeapon *pWeapon = (CBasePlayerWeapon *)m_pActiveItem->GetWeaponPtr();
                if( pWeapon )
                        pWeapon->m_flTimeWeaponIdle = UTIL_WeaponTimeBase() + 1.0f;

                // For a regular save/load (not a landmark changelevel) the
                // client IS already connected; re-deploy so the view model,
                // weapon model and HUD are refreshed correctly.
                // During changelevel (fUseLandmark == TRUE) the client is not
                // connected yet — Deploy() would deadlock via SVC_WEAPONANIM.
                if( !pSaveData->fUseLandmark )
