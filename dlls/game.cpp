...
// Slow-motion: pointer to the engine's sys_timescale cvar (always present in Xash3D)
...
cvar_t max_commentary               = { "max_commentary",              "1", FCVAR_ARCHIVE };
cvar_t max_commentary_pkup          = { "max_commentary_painkiller_pickup", "1", FCVAR_ARCHIVE };
cvar_t max_commentary_puse          = { "max_commentary_painkiller_use",    "1", FCVAR_ARCHIVE };
cvar_t max_commentary_pain          = { "max_commentary_pain",          "1", FCVAR_ARCHIVE };
cvar_t max_commentary_pain_self     = { "max_commentary_pain_self",     "1", FCVAR_ARCHIVE };
cvar_t max_commentary_kill_innocent = { "max_commentary_kill_innocent", "1", FCVAR_ARCHIVE };
cvar_t max_commentary_no_ammo       = { "max_commentary_no_ammo",       "1", FCVAR_ARCHIVE };
cvar_t max_commentary_wasted_shots  = { "max_commentary_wasted_shots",  "1", FCVAR_ARCHIVE };
cvar_t max_commentary_near_death    = { "max_commentary_near_death",    "1", FCVAR_ARCHIVE };
...
void GameDLLInit( void )
{
	...
	// Xash3D always exposes sys_timescale; grab the pointer so we can drive slow-motion.
	g_sys_timescale = CVAR_GET_POINTER( "sys_timescale" );
	using_sys_timescale = ( g_sys_timescale != NULL );
	if ( !using_sys_timescale )
		g_engfuncs.pfnServerPrint( "sys_timescale cvar not found - slow-motion disabled\n" );

	CVAR_REGISTER( &build_commit );
	CVAR_REGISTER( &build_branch );

	CVAR_REGISTER( &max_commentary );
	CVAR_REGISTER( &max_commentary_pkup );
	CVAR_REGISTER( &max_commentary_puse );
	CVAR_REGISTER( &max_commentary_pain );
	CVAR_REGISTER( &max_commentary_pain_self );
	CVAR_REGISTER( &max_commentary_kill_innocent );
	CVAR_REGISTER( &max_commentary_no_ammo );
	CVAR_REGISTER( &max_commentary_wasted_shots );
	CVAR_REGISTER( &max_commentary_near_death );
