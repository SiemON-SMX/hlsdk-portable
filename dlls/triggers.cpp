...
void CMultiManager::ManagerThink( void )
{
        ...
        {
                // c5a1 desperation system: intercept start_loser_mm to begin the finale sequence
                if( strcmp( STRING( gpGlobals->mapname ), "c5a1" ) == 0 &&
                    FStrEq( STRING( m_iTargetName[m_index] ), "start_loser_mm" ) )
                {
                        CBasePlayer *pPlayer = static_cast<CBasePlayer *>( CBaseEntity::Instance( g_engfuncs.pfnPEntityOfEntIndex( 1 ) ) );
                        if( pPlayer && pPlayer->IsPlayer() &&
                            pPlayer->desperation < CBasePlayer::DESPERATION_REVENGE )
                        {
                                pPlayer->desperation = CBasePlayer::DESPERATION_PRE_IMMINENT;
...
static const char * const g_cdTrackMap[] =
{
        "",                              // 0 - unused
        "",                              // 1 - unused
        "valve/media/Half-Life01.mp3",   // 2
        "valve/media/Prospero01.mp3",    // 3
        "valve/media/Half-Life12.mp3",   // 4
        "valve/media/Half-Life07.mp3",   // 5
        "valve/media/Half-Life10.mp3",   // 6
        "valve/media/Suspense01.mp3",    // 7
        "valve/media/Suspense03.mp3",    // 8
        "valve/media/Half-Life09.mp3",   // 9
        "valve/media/Half-Life02.mp3",   // 10
        "valve/media/Half-Life13.mp3",   // 11
        "valve/media/Half-Life04.mp3",   // 12
        "valve/media/Half-Life15.mp3",   // 13
        "valve/media/Half-Life14.mp3",   // 14
        "valve/media/Half-Life16.mp3",   // 15
        "valve/media/Suspense02.mp3",    // 16
        "valve/media/Half-Life03.mp3",   // 17
        "valve/media/Half-Life08.mp3",   // 18
        "valve/media/Prospero02.mp3",    // 19
        "valve/media/Half-Life05.mp3",   // 20
        "valve/media/Prospero04.mp3",    // 21
        "valve/media/Half-Life11.mp3",   // 22
        "valve/media/Half-Life06.mp3",   // 23
        "valve/media/Prospero03.mp3",    // 24
        "valve/media/Half-Life17.mp3",   // 25
        "valve/media/Prospero05.mp3",    // 26
        "valve/media/Suspense05.mp3",    // 27
        "valve/media/Suspense07.mp3",    // 28
};
#define NUM_CD_TRACKS ( (int)( sizeof( g_cdTrackMap ) / sizeof( g_cdTrackMap[0] ) ) )
...
void PlayCDTrack( int iTrack )
{
        ...
        if( iTrack < -1 || iTrack >= NUM_CD_TRACKS )
        {
        ...
        else if( g_cdTrackMap[iTrack][0] != '\0' )
        {
                char cmd[128];
...
void CBaseTrigger::ActivateMultiTrigger( CBaseEntity *pActivator )
{
        ...
        // Commentary hook: fire when player walks through this trigger
        if ( pActivator && pActivator->IsPlayer() )
                HalfPayneHookModelIndex( ENT( pev ) );
...
void CBaseTrigger::TeleportTouch( CBaseEntity *pOther )
{
        ...
        // c5a1 desperation system: "backtrain" teleport triggers ALL_FOR_REVENGE
        if( pOther->IsPlayer() &&
            FStrEq( STRING( gpGlobals->mapname ), "c5a1" ) &&
            !FStringNull( pev->target ) &&
            FStrEq( STRING( pev->target ), "backtrain" ) )
        {
                CBasePlayer *pPlayer = static_cast<CBasePlayer *>( pOther );
                if( pPlayer->desperation != CBasePlayer::DESPERATION_ALL_FOR_REVENGE &&
                    pPlayer->desperation != CBasePlayer::DESPERATION_REVENGE )
                {
                        pPlayer->desperation = CBasePlayer::DESPERATION_ALL_FOR_REVENGE;
                        pPlayer->untilNextDesperation = gpGlobals->time + 1.6f;
                        pPlayer->pev->gravity = 1.0f;
                        pPlayer->AddToSoundQueue( ALLOC_STRING( "comment/execute.wav" ), 0.6f );
                }
        }
