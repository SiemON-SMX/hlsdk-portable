...
#include <string>
#include <vector>
#define RGB_YELLOWISH 0x00FFFFFF //255,255,255 (white HUD: weapons / health / ammo / clip)
#define RGB_REDISH 0x00FF1010 //255,16,16  (kept red for low-health / out-of-ammo warnings)
...
public:
        ...
        int MsgFunc_TwinClip( const char *pszName, int iSize, void *pbuf );
        int MsgFunc_AimCoords( const char *pszName, int iSize, void *pbuf );
        int DrawAimCoords();
        float aimCoordX, aimCoordY, aimCoordZ, aimCoordAngle;
...
private:
        ...
        int m_HUD_selection;
public:
        int m_iTwinLeftClip;     // ammo in the LEFT pistol of weapon_9mmhandgun_twin (right is m_pWeapon->iClip)
};
...
//-----------------------------------------------------
// Slow-motion charge indicator (hourglass)
//-----------------------------------------------------
...
private:
        struct EndAnimLine { std::string label; std::string value; };
        struct EndStatLine { std::string key;   std::string value; };
        std::vector<std::string>  titleLines;
        std::vector<EndAnimLine>  animLines;
        std::vector<EndStatLine>  statLines;
        bool cheated;
};
...
private:
        int XPosition( float x, int width, int totalWidth );
        int YPosition( float y, int height );
