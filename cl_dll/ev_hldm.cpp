...
static void EV_FireGlock_Impl( event_args_t *args )
{
        ...
        {
                EV_MuzzleFlash();
                gEngfuncs.pEventAPI->EV_WeaponAnimation( empty ? GLOCK_SHOOT_THEN_EMPTY : GLOCK_SHOOT, 0 );
...
void EV_FireGlockTwin( event_args_t *args )
{
        ...
        if( EV_IsLocal( idx ) )
        {
                int iAnim;
                if( bLeft )
                        iAnim = empty ? 16 /* shoot_left_then_empty */ : 12 /* shoot_left */;
                else
                        iAnim = empty ? 8  /* shoot_right_then_empty */ : 4  /* shoot_right */;
                ...
                if( vm )
                {
                        const int iAttach = bLeft ? 1 : 0;
                        gEngfuncs.pEfxAPI->R_MuzzleFlash( (float *)&vm->attachment[iAttach], 0 );
                }
        ...
        EV_GetDefaultShellInfo( args, origin, velocity, ShellVelocity, ShellOrigin,
                forward, right, up, 20, bLeft ? 12 : -12, 4 );

        EV_EjectBrass( ShellOrigin, ShellVelocity, angles[YAW], shell, TE_BOUNCE_SHELL );

        gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/pl_gun3.wav",
        ...
        EV_HLDM_FireBullets( idx, forward, right, up, 1, vecSrc, vecAiming, 8192,
                BULLET_PLAYER_9MM, 0, &g_tracerCount[idx - 1], args->fparam1, args->fparam2 );
}
...
void EV_FireM249( event_args_t *args )
{
        ...
        // bparam1 picks alternating brass model; both fall back to standard shell.
        shell = gEngfuncs.pEventAPI->EV_FindModelIndex( "models/shell.mdl" );
        ...
        if( EV_IsLocal( idx ) )
        {
                ...
                gEngfuncs.pEventAPI->EV_WeaponAnimation(
                        M249L_SHOOT1 + gEngfuncs.pfnRandomLong( 0, 2 ),
                        args->iparam2 );
        ...
        gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON, "weapons/saw_fire1.wav",
                1, ATTN_NORM, 0, PITCH_NORM );

        EV_GetGunPosition( args, vecSrc, origin );
        VectorCopy( forward, vecAiming );

        EV_HLDM_FireBullets( idx, forward, right, up, 1, vecSrc, vecAiming, 8192,
                BULLET_PLAYER_MP5, 2, &g_tracerCount[idx - 1], args->fparam1, args->fparam2 );
}
...
{
        EV_ITWIN_IDLE = 0,
        EV_ITWIN_IDLE_NOSHOT_BOTH,
        EV_ITWIN_IDLE_NOSHOT_LEFT,
        EV_ITWIN_IDLE_NOSHOT_RIGHT,
        EV_ITWIN_SHOOT_BOTH_1,
        EV_ITWIN_SHOOT_BOTH_2,
        EV_ITWIN_SHOOT_BOTH_3,
        EV_ITWIN_SHOOT_BOTH_THEN_EMPTY,
        EV_ITWIN_SHOOT_BOTH_THEN_LEFT_EMPTY,
        EV_ITWIN_SHOOT_BOTH_THEN_RIGHT_EMPTY,
        EV_ITWIN_SHOOT_RIGHT_WHEN_LEFT_EMPTY_1,
        EV_ITWIN_SHOOT_RIGHT_WHEN_LEFT_EMPTY_2,
        EV_ITWIN_SHOOT_RIGHT_WHEN_LEFT_EMPTY_3,
        EV_ITWIN_SHOOT_RIGHT_THEN_EMPTY_WHEN_LEFT_EMPTY,
        EV_ITWIN_SHOOT_LEFT_WHEN_RIGHT_EMPTY_1,
        EV_ITWIN_SHOOT_LEFT_WHEN_RIGHT_EMPTY_2,
        EV_ITWIN_SHOOT_LEFT_WHEN_RIGHT_EMPTY_3,
        EV_ITWIN_SHOOT_LEFT_THEN_EMPTY_WHEN_RIGHT_EMPTY,

        EV_ITWIN_RESERVED_18,
        EV_ITWIN_RESERVED_19,
        EV_ITWIN_RESERVED_20,
        EV_ITWIN_RESERVED_21,
        EV_ITWIN_RESERVED_22,
        EV_ITWIN_RESERVED_23,
        EV_ITWIN_RESERVED_24,
        EV_ITWIN_RESERVED_25,
        EV_ITWIN_RESERVED_26,
        EV_ITWIN_RESERVED_27,
        EV_ITWIN_RESERVED_28,
        EV_ITWIN_RESERVED_29,
        EV_ITWIN_RESERVED_30,
        EV_ITWIN_RESERVED_31,
        EV_ITWIN_RESERVED_32,
        EV_ITWIN_RESERVED_33,
        EV_ITWIN_RESERVED_34,
        EV_ITWIN_RESERVED_35,
        EV_ITWIN_RESERVED_36,
        EV_ITWIN_RESERVED_37,
        EV_ITWIN_RESERVED_38,
        EV_ITWIN_RESERVED_39,
        EV_ITWIN_RESERVED_40,
        EV_ITWIN_RESERVED_41,
        EV_ITWIN_RESERVED_42,
        EV_ITWIN_RESERVED_43,
        EV_ITWIN_RESERVED_44,
        EV_ITWIN_RESERVED_45,
        EV_ITWIN_RESERVED_46,
        EV_ITWIN_RESERVED_47,
        EV_ITWIN_RESERVED_48,
        EV_ITWIN_RESERVED_49,
        EV_ITWIN_RESERVED_50,
        EV_ITWIN_RESERVED_51,
...
void EV_FireIngram( event_args_t *args )
{
        ...
        if( EV_IsLocal( idx ) )
        {
                EV_MuzzleFlash();
                gEngfuncs.pEventAPI->EV_WeaponAnimation(
                        empty ? EV_ING_SHOOT_EMPTY
                              : ( EV_ING_SHOOT_1 + gEngfuncs.pfnRandomLong( 0, 2 ) ),
                        2 );
                V_PunchAxis( 0, gEngfuncs.pfnRandomFloat( -2.0f, -1.0f ) );
        }

        EV_GetDefaultShellInfo( args, origin, velocity, ShellVelocity, ShellOrigin,
                forward, right, up, 20, -8, 4 );
        EV_EjectBrass( ShellOrigin, ShellVelocity, angles[YAW], shell, TE_BOUNCE_SHELL );

        gEngfuncs.pEventAPI->EV_PlaySound( idx, origin, CHAN_WEAPON,
                "weapons/ingram_shot.wav",
        ...
        EV_HLDM_FireBullets( idx, forward, right, up, 1, vecSrc, vecAiming, 8192,
                BULLET_PLAYER_9MM, 1, &g_tracerCount[idx - 1],
...
void EV_FireIngramTwin( event_args_t *args )
{
        ...
        const int iEmptyFlags = args->iparam1;   // bit0 = right empty, bit1 = left empty (post-shot)
        const bool bRightEmpty = ( iEmptyFlags & 1 ) != 0;
        const bool bLeftEmpty  = ( iEmptyFlags & 2 ) != 0;

        // bparam2: 0 = right only, 1 = left only, 2 = both
        const int iSide = args->bparam2;
        const bool bShotRight = ( iSide == 0 || iSide == 2 );
        const bool bShotLeft  = ( iSide == 1 || iSide == 2 );
        ...
        if( EV_IsLocal( idx ) )
        {
                ...
                if( iSide == 2 )
                {
                        if( bRightEmpty && bLeftEmpty )
                                iAnim = EV_ITWIN_SHOOT_BOTH_THEN_EMPTY;
                        else if( bLeftEmpty )
                                iAnim = EV_ITWIN_SHOOT_BOTH_THEN_LEFT_EMPTY;
                        else if( bRightEmpty )
                                iAnim = EV_ITWIN_SHOOT_BOTH_THEN_RIGHT_EMPTY;
                        else
                                iAnim = EV_ITWIN_SHOOT_BOTH_1 + gEngfuncs.pfnRandomLong( 0, 2 );
                }
                else if( iSide == 0 ) // shooting right only (left already empty)
                {
                        iAnim = bRightEmpty
                                ? EV_ITWIN_SHOOT_RIGHT_THEN_EMPTY_WHEN_LEFT_EMPTY
                                : ( EV_ITWIN_SHOOT_RIGHT_WHEN_LEFT_EMPTY_1 + gEngfuncs.pfnRandomLong( 0, 2 ) );
                }
                else // shooting left only (right already empty)
                {
                        iAnim = bLeftEmpty
                                ? EV_ITWIN_SHOOT_LEFT_THEN_EMPTY_WHEN_RIGHT_EMPTY
                                : ( EV_ITWIN_SHOOT_LEFT_WHEN_RIGHT_EMPTY_1 + gEngfuncs.pfnRandomLong( 0, 2 ) );
                }

                EV_MuzzleFlash();
                gEngfuncs.pEventAPI->EV_WeaponAnimation( iAnim, 2 );

                // Explicit second-side muzzle-flash sprite when both fire.
                cl_entity_t *vm = gEngfuncs.GetViewModel();
                if( vm )
                {
                        if( bShotLeft )
                                gEngfuncs.pEfxAPI->R_MuzzleFlash( (float *)&vm->attachment[1], 0 );
                }

                V_PunchAxis( 0, gEngfuncs.pfnRandomFloat( -2.5f, -1.5f ) );
        }
        ...
        // Bullets / tracers travel in the per-side tracer event below.
}
