...
int CHudMessage::VidInit( void )
{
        ...
        m_HUD_title_credits1 = gHUD.GetSpriteIndex( "title_halfpayne" );
...
int CHudMessage::Draw( float fTime )
{
        ...
        if( m_gameTitleTime > 0 )
        {
                ...
                else
                {
                        ...
                        if( m_HUD_title_credits1 > 0 )
                        {
                                wrect_t logoRect = gHUD.GetSpriteRect( m_HUD_title_credits1 );
                                int fullWidth  = logoRect.right - logoRect.left;
                                int fullHeight = logoRect.bottom - logoRect.top;

                                int x = XPosition( m_pGameTitle->x, fullWidth, fullWidth );
                                int y = YPosition( m_pGameTitle->y, fullHeight );

                                SPR_Set( gHUD.GetSprite( m_HUD_title_credits1 ), brightness * m_pGameTitle->r1, brightness * m_pGameTitle->g1, brightness * m_pGameTitle->b1 );
                                SPR_DrawAdditive( 0, x, y, &logoRect );
                        }
                        else
                        {
