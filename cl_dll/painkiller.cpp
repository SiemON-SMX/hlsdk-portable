...
DECLARE_MESSAGE( m_Painkiller, PillCount )
...
int CHudPainkiller::Draw( float flTime )
{
        ...
        int maxSpriteW = XRES( PAINKILLER_SPRITE_WIDTH );
        if( ( painkillerRect.bottom - painkillerRect.top ) > maxSpriteH )
                painkillerRect.bottom = painkillerRect.top + maxSpriteH;
        if( ( painkillerRect.right - painkillerRect.left ) > maxSpriteW )
                painkillerRect.right = painkillerRect.left + maxSpriteW;
        ...
        int x = XRES( CORNER_OFFSET ) + HEALTH_SPRITE_WIDTH + BOTTOM_LEFT_SPACING
              + HOURGLASS_SPRITE_WIDTH + BOTTOM_LEFT_SPACING;
        int y = ScreenHeight - painkillerRectHeight - YRES( CORNER_OFFSET );

        SPR_Set( gHUD.GetSprite( painKillerSprite ), 255, 255, 255 );
        SPR_Draw( 0, x, y, &painkillerRect );

        x -= PAINKILLER_SPRITE_WIDTH + 4;
        y = ScreenHeight - YRES( CORNER_OFFSET ) - gHUD.m_iFontHeight - 2;
