...
DECLARE_MESSAGE( m_Ammo, TwinClip )
...
int CHudAmmo::MsgFunc_TwinClip( const char *pszName, int iSize, void *pbuf )
{
        BEGIN_READ( pbuf, iSize );
        m_iTwinLeftClip = READ_BYTE();
        return 1;
}
...
int CHudAmmo::DrawAimCoords()
{
        ...
        int _len = gHUD.DrawHudStringLen( aimCoords );
        gHUD.DrawHudString( x - _len / 2, y, x + _len, aimCoords, 200, 200, 200 );
        return 1;
}
...
int CHudAmmo::Draw( float flTime )
{
        ...
        if( m_pWeapon->iAmmoType > 0 )
        {
                ...
                if( m_pWeapon->iId == 16 /* WEAPON_GLOCK_TWIN */ )
                {
                        int yLeft = y - gHUD.m_iFontHeight - gHUD.m_iFontHeight / 4;
                        int xLeft = ScreenWidth - ( 8 * AmmoWidth ) - iIconWidth;

                        int rL, gL, bL, aL = a;
                        UnpackRGB( rL, gL, bL, RGB_YELLOWISH );
                        ScaleColors( rL, gL, bL, aL );

                        xLeft = gHUD.DrawHudNumber( xLeft, yLeft, iFlags | DHN_3DIGITS, m_iTwinLeftClip, rL, gL, bL );

                        int iBarWidth = AmmoWidth / 10;
                        xLeft += AmmoWidth / 2;
                        UnpackRGB( rL, gL, bL, RGB_YELLOWISH );
                        FillRGBA( xLeft, yLeft, iBarWidth, gHUD.m_iFontHeight, rL, gL, bL, aL );
                        xLeft += iBarWidth + AmmoWidth / 2;

                        ScaleColors( rL, gL, bL, aL );
                        // "L" marker drawn via the small ammo icon so the player knows this row is the left clip
                        SPR_Set( m_pWeapon->hAmmo, rL, gL, bL );
                        SPR_DrawAdditive( 0, xLeft, yLeft - iOffset, &m_pWeapon->rcAmmo );
                }
        }
